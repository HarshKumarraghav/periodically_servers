...
Array.prototype.paginate = function (page = 1, limit = this.length) {
  ...
  page--;
  if (
    this.slice(page * limit, page * limit + limit).length > 0 ||
    this.length > 0
  )
    return this.slice(page * limit, page * limit + limit);
  else
    return {
      error: "not found.",
    };
...
app.get("/period/:period", (req, res) => {
  res.json(
    queryParams(
      req.query,
      elements.filter((element) => element.period == req.params.period)
    )
  );
});
...
app.get("/phase/:phase", (req, res) => {
  res.json(
    queryParams(
      req.query,
      elements.filter((element) => element.phase == req.params.phase)
    )
  );
});
...
app.get("/name/:name", (req, res) => {
  res.json(
    elements.filter((element) => element.name == req.params.name).paginate(1, 1)
  );
});
...
app.get("/number/:number", (req, res) => {
  res.json(
    elements
      .filter((element) => element.number == req.params.number)
      .paginate(1, 1)
  );
});
...
app.get("/symbol/:symbol", (req, res) => {
  res.json(
    elements
      .filter((element) => element.symbol == req.params.symbol)
      .paginate(1, 1)
  );
});
...
app.get("/category/:name", (req, res) => {
  res.json(
    elements
      .filter((element) => element.category == req.params.name)
      .paginate(1, 1)
  );
});
...
app.get("/spectral/:element", (req, res) => {
  if (
    req.params.element.length <= 2 &&
    elements.find((element) => element.symbol == req.params.element)
      .spectral_img != null
  ) {
    res.redirect(
      elements.find((element) => element.symbol == req.params.element)
    ...
    req.params.element.length > 3 &&
    elements.find((element) => element.name == req.params.element)
      .spectral_img != null
  ) {
    res.redirect(
      elements.find((element) => element.name == req.params.element)
